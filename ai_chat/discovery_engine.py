...
def _build_candidates_text(books: list[Book]) -> str:
    ...
    for i, book in enumerate(books, 1):
        ...
        line = f"{i}. (ID: {book.pk}) «{book.title}»"
        if authors:
            line += f" — {authors}"
        if genres:
            line += f" [{genres}]"
...
def ask_discovery(user, message: str, chat: DiscoveryChat,
    ...
    messages = [{
        ...
        "content": (
            ...
            "• Рекомендуй и упоминай в своем текстовом ответе (explanation) ТОЛЬКО те книги, которые строго присутствуют в предоставленном ниже каталоге (это книги, реально существующие в нашей базе данных). Категорически запрещено советовать, рекомендовать или упоминать любые книги, которых нет в этом каталоге.\n"
            "• Любое упоминание книги в твоем тексте (explanation) должно быть оформлено строго в формате [book:ID|Название], используя её реальный ID, указанный рядом с номером в каталоге (например: [book:3|Преступление и наказание]). Это автоматически сделает её кликабельной для пользователя. Не используй обычный текст или кавычки для названий книг — только формат [book:ID|Название].\n\n"
